...
async function fetchChat() {
    ...
    // Build HTML string to prevent DOM flickering
    let newHTML = '';
    data.forEach(msg => {
        newHTML += `
            <div class="chat-msg">
...
supabase.channel('public:chat_messages')
  .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, payload => {
     // Append only the new message to avoid re-rendering the whole list
     const chatBox = document.getElementById('chat-messages');
...
document.addEventListener("DOMContentLoaded", () => {
    // Generate sleek canvas gradients for the waveform
    ...
    waveGradient.addColorStop(0, 'rgba(255, 255, 255, 0.6)'); // Brighter top
    waveGradient.addColorStop(1, 'rgba(255, 255, 255, 0.3)'); // Much less dark bottom

    const progressGradient = ctxGrad.createLinearGradient(0, 0, 0, 70);
    progressGradient.addColorStop(0, '#e08fff'); // Lighter purple top
    progressGradient.addColorStop(1, '#c552ff'); // Brighter, less dark purple bottom
