...
// ==========================================
// CONFIG & BACKGROUND HELPERS (CACHED)
// ==========================================
...
async function loadPageBackground() {
    ...
    // 1. Instantly load from local memory to completely prevent the "flash"
    const cachedBg = localStorage.getItem('cached_background');
    if (cachedBg) {
        bgElement.style.backgroundImage = `url('${cachedBg}')`;
    }

    // 2. Fetch the true current background from the database
    const bgUrl = await getConfigValue('background_image_url');
    const finalUrl = bgUrl || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop';
    
    // 3. Apply it smoothly and update the cache for next time
    if (cachedBg !== finalUrl) {
        setTimeout(() => {
            bgElement.style.backgroundImage = `url('${finalUrl}')`;
        }, 50);
        localStorage.setItem('cached_background', finalUrl);
    }
}
...
supabase.channel('public:config')
  .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'config' }, payload => {
     ...
     if (payload.new.key === 'background_image_url') loadPageBackground(); 
  }).subscribe();
...
document.addEventListener("DOMContentLoaded", () => {
    // Dynamic height based on screen size to fix mobile overflow
    const waveHeight = window.innerWidth <= 768 ? 90 : 140;
...
window.handleBackgroundUpload = async (e) => {
    ...
    try {
        ...
        const { error: dbError } = await supabase.from('config').upsert([{ key: 'background_image_url', value: urlData.publicUrl }]);
        if (dbError) throw new Error("Database Error: " + dbError.message + " (Check RLS policies on your 'config' table)");
