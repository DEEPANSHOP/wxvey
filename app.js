...
// ==========================================
// LIVE CHAT LOGIC (INSTANT UI UPDATE)
// ==========================================
...
window.toggleChat = () => {
    ...
    document.body.classList.toggle('chat-open'); // Toggles body push effect
    
    if(chatPanel.classList.contains('open')) {
        const chatBox = document.getElementById('chat-messages');
        chatBox.scrollTop = chatBox.scrollHeight; // Auto-scroll on open
    }
};
...
supabase.channel('public:chat_messages')
  .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, payload => {
     // Re-fetch to guarantee sync and replace the temporary faded message
     fetchChat(); 
  })
  .subscribe();
...
window.sendChatMessage = async (e) => {
    ...
    // 1. INSTANT LOCAL UPDATE (Shows immediately)
    const chatBox = document.getElementById('chat-messages');
    if(chatBox.innerHTML.includes('> NO MESSAGES YET.')) chatBox.innerHTML = '';
    
    const msgDiv = document.createElement('div');
    msgDiv.className = 'chat-msg';
    msgDiv.style.opacity = '0.5'; // Faded slightly to indicate it's "sending"
    ...
    if (error) {
        ...
        msgDiv.remove(); // Remove it if it failed to send
...
// ==========================================
// WAVESURFER & AUDIO TIMERS (GRADIENT FIX)
// ==========================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const waveGradient = ctxGrad.createLinearGradient(0, 0, 0, 70);
    waveGradient.addColorStop(0, 'rgba(255, 255, 255, 0.6)'); 
    waveGradient.addColorStop(1, 'rgba(255, 255, 255, 0.3)'); 

    const progressGradient = ctxGrad.createLinearGradient(0, 0, 0, 70);
    progressGradient.addColorStop(0, '#e08fff'); 
    progressGradient.addColorStop(1, '#c552ff'); 
